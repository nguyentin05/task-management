...
Thư mục này chứa các **sơ đồ tuần tự (Sequence Diagram)** mô tả luồng tương tác giữa các thành phần hệ thống khi thực hiện các ca sử dụng quan trọng trong hệ thống Task Management.
...
sequence/
├── images/
│   ├── structurizr-RegisterUser.svg      # Đăng ký người dùng
│   ├── structurizr-DeleteTask.svg        # Xóa task
│   ├── structurizr-GetAllColumns.svg     # Lấy danh sách cột Kanban
│   ├── structurizr-SearchToInvite.svg    # Tìm kiếm user để mời vào dự án
│   └── structurizr-UpdateAvatar.svg      # Cập nhật ảnh đại diện
├── mermaid/
│   ├── structurizr-RegisterUser.mmd
│   ├── structurizr-DeleteTask.mmd
│   ├── structurizr-GetAllColumns.mmd
│   ├── structurizr-SearchToInvite.mmd
│   └── structurizr-UpdateAvatar.mmd
├── plantuml/
│   ├── structurizr-RegisterUser.puml
│   ├── structurizr-DeleteTask.puml
│   ├── structurizr-GetAllColumns.puml
│   ├── structurizr-SearchToInvite.puml
│   └── structurizr-UpdateAvatar.puml
└── wsd/
    ├── structurizr-RegisterUser.wsd
    ├── structurizr-DeleteTask.wsd
    ├── structurizr-GetAllColumns.wsd
    ├── structurizr-SearchToInvite.wsd
...
> Nguồn: Sinh từ [Structurizr](https://structurizr.com/) và export sang các định dạng `.mmd`, `.puml`, `.wsd`.
