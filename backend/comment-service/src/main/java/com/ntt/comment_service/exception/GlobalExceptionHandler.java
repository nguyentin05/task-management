...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ApiResponse<?>> handlingValidationException(MethodArgumentNotValidException exception) {
        ...
        try {
            ...
            log.warn("[Comment][Controller]Không tìm thấy mã lỗi tương ứng: {}", enumKey);
        } catch (Exception e) {
            log.warn("[Comment][Controller]Không thể trích xuất attributes validation cho mã lỗi: {}", enumKey, e);
        }
    ...
    ResponseEntity<ApiResponse<?>> handlingDbException(DataIntegrityViolationException exception) {
        log.error(
                "[Comment][DB] Lỗi ràng buộc dữ liệu: {}",
    ...
    ResponseEntity<ApiResponse<?>> handlingDataAccessException(DataAccessException exception) {
        log.error("[Comment][INFRA] Lỗi không thể kết nối db: {}", exception.getMessage(), exception);
    ...
    ResponseEntity<ApiResponse<?>> handlingException(Exception exception) {
        log.error("[Comment][Global]Lỗi chưa được phân loại: {}", exception.getMessage());
        return buildResponse(ErrorCode.INTERNAL_SERVER_ERROR);
    }
