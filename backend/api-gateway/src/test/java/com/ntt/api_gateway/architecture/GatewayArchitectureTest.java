...
public class GatewayArchitectureTest {
    ...
    static final ArchRule filters_should_be_in_correct_package_and_named = classes()
            .that()
            .implement(org.springframework.cloud.gateway.filter.GlobalFilter.class)
            .or()
            .implement(org.springframework.cloud.gateway.filter.GatewayFilter.class)
            .should()
    ...
    static final ArchRule no_webmvc_dependencies = noClasses()
            ...
            .because(
                    "API Gateway chạy trên WebFlux, tuyệt đối không import thư viện của WebMVC (như HttpServletRequest)");
}
